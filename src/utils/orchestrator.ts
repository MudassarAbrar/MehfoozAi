...
export async function processSafetyOrchestration(
  ...
  // 4. Out of scope (other provinces)
  if (intent === 'out_of_scope') {
    return {
      intent: 'out_of_scope',
      riskLevel: 'informational',
      language: effectiveLang,
      answerSummary: 'Notice: Mehfooz is currently grounded strictly in the laws and institutions of Punjab, Pakistan (such as the Punjab Protection of Women Against Violence Act 2016 and Punjab Safe Cities Authority). While general federal laws (such as Pakistan Penal Code and Workplace Harassment Act) apply nationwide, specific provincial protection committees and shelters referenced here are specific to Punjab.',
  ...
  // 5. Search grounded Punjab Legal Corpus
  const relevantCitations = searchLegalCorpus(userQuery, 3);

  // 6. If offline or no network connection, immediately synthesize using deterministic local grounded corpus
  if (typeof navigator !== 'undefined' && !navigator.onLine) {
    if (intent === 'immediate_danger') {
      return getImmediateDangerFallback(effectiveLang);
    }
  ...
  // If immediate danger and server did not respond, return emergency safety alert
  if (intent === 'immediate_danger') {
    return getImmediateDangerFallback(effectiveLang);
  }

  // 8. Intelligent Client-Side Grounded RAG Synthesis Fallback
  return generateClientGroundedResponse(userQuery, effectiveLang, intent, risk, relevantCitations);
}

function getImmediateDangerFallback(effectiveLang: AppLanguage): LegalQueryResponse {
  const isUrdu = effectiveLang === 'ur';
  return {
...
function generateClientGroundedResponse(
  ...
  return {
    ...
    confidence: 0.94,
    disclaimerRequired: true,
    modelUsed: 'offline-local-grounding',
    isAiGenerated: false,
    suggestedActions: [
