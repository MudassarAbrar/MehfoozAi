...
export const PDF_LABELS = {
  petitionDraftTitle: 'Mehfooz — User-Generated Protective Petition Draft',
  petitionDraftSubtitle: 'Prepared for submission to the relevant Punjab authority',
  petitionRunningHeader: 'MEHFOOZ • USER-GENERATED PROTECTIVE PETITION DRAFT',
  petitionFooterTag: 'USER-GENERATED DRAFT • FOR SUBMISSION REVIEW ONLY',
  petitionDisclaimer: 'NOTICE: This document is generated by the user through Mehfooz and is intended as a draft for potential submission to the relevant authority. It is not an official Government of Punjab document and has not been issued, approved, or verified by a government authority.',
  
  protectedRecordTitle: 'Protected Incident Record',
  protectedRecordSubtitle: 'Private Contemporaneous Incident Logs Compiled via Mehfooz',
  protectedRecordRunningHeader: 'MEHFOOZ • PROTECTED INCIDENT RECORD',
  protectedRecordFooterTag: 'PROTECTED INCIDENT RECORD • STRICTLY PRIVATE'
};
...
function drawPageDecorations(
  ...
  refCode: string, 
  isEncrypted: boolean,
  headerTitle = PDF_LABELS.petitionRunningHeader,
  footerTag = PDF_LABELS.petitionFooterTag
) {
  ...
  doc.text(headerTitle, 16, 17);
  doc.text(`REF: ${refCode}`, pageWidth - 16, 17, { align: 'right' });

  // Bottom footer text
  doc.setFontSize(8);
  doc.setTextColor(100, 116, 139);
  const securityTag = isEncrypted ? ' • [PASSWORD-PROTECTED PDF DOCUMENT]' : '';
  doc.text(
    `${footerTag}${securityTag}`,
    16,
    pageHeight - 16
  );
...
export async function exportComplaintToPDF(
  ...
  // 1. PETITION DRAFT MAIN HEADER
  doc.setFont('times', 'bold');
  doc.setFontSize(13);
  doc.setTextColor(15, 23, 42); // slate-900
  doc.text(PDF_LABELS.petitionDraftTitle, pageWidth / 2, cursorY, { align: 'center' });
  cursorY += 5;

  doc.setFont('times', 'italic');
  doc.setFontSize(9.5);
  doc.setTextColor(71, 85, 105);
  doc.text(PDF_LABELS.petitionDraftSubtitle, pageWidth / 2, cursorY, { align: 'center' });
  cursorY += 5.5;

  // Intended Recipient Authority Heading
  const authority = getAuthorityHeading(draft.requestedSupport, draft.officialChannelUsed);
  doc.setFont('times', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(71, 85, 105);
  doc.text('INTENDED RECIPIENT AUTHORITY:', pageWidth / 2, cursorY, { align: 'center' });
  cursorY += 4.5;

  doc.setFont('times', 'bold');
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  const authLines = doc.splitTextToSize(authority, contentWidth - 10);
  doc.text(authLines, pageWidth / 2, cursorY, { align: 'center' });
  cursorY += authLines.length * 4.5 + 2;

  // Disclaimer Box
  doc.setFillColor(254, 242, 242); // rose-50/amber-50 blend
  doc.setDrawColor(252, 165, 165);
  const disclaimerLines = doc.splitTextToSize(PDF_LABELS.petitionDisclaimer, contentWidth - 8);
  const boxHeight = disclaimerLines.length * 3.8 + 4;
  doc.rect(margin, cursorY, contentWidth, boxHeight, 'FD');

  doc.setFont('times', 'normal');
  doc.setFontSize(7.5);
  doc.setTextColor(153, 27, 27);
  doc.text(disclaimerLines, margin + 4, cursorY + 3.8);
  cursorY += boxHeight + 4;
...
export async function exportIncidentRecordsToPDF(
  ...
  doc.setFontSize(13);
  doc.setTextColor(15, 23, 42);
  doc.text(PDF_LABELS.protectedRecordTitle, pageWidth / 2, cursorY, { align: 'center' });
  cursorY += 5;

  doc.setFont('times', 'italic');
  doc.setFontSize(9);
  doc.setTextColor(71, 85, 105);
  doc.text(PDF_LABELS.protectedRecordSubtitle, pageWidth / 2, cursorY, { align: 'center' });
  cursorY += 5;
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    drawPageDecorations(
      ...
      refCode, 
      hasPassword, 
      PDF_LABELS.protectedRecordRunningHeader,
      PDF_LABELS.protectedRecordFooterTag
    );
  }

  const fileName = `Mehfooz_Protected_Incident_Record_${new Date().toISOString().split('T')[0]}.pdf`;
  const blob = doc.output('blob');
  const base64 = doc.output('datauristring');
